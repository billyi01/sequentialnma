#' Continuously updated network meta-analysis
#'
#' This function provides methods for continuously updating pairwise and network meta.analysis.
#' The methodology uses formal statistical monitoring initially suggested for clinical trials and
#' pairwise meta-analysis. It is described in detail in Nikolakopoulou A, Mavridis D, Egger M, Salanti G. 
#' Continuously updated network meta-analysis and statistical monitoring for timely decision-making. 
#' Stat Methods Med Res. 2016 Sep 1; 
#' @param data a csv file saved in the work directory. For arm level binary data,
#' the required variables are "year study id t r n". For arm level continuous data, the required
#' variables are "year study id t y sd n". For study level data of any type, the required variables
#' are "year study id t1 t2 effect se" where "effect" is a treatment effect on an additive scale,
#' e.g. mean difference, log odds ratio, log hazard ratio.
#' @param level A character string indicating the level of the data, it can be either "arm"
#' if data are available for each treatment arm or "study" if data are avaiable per comparison
#' @param type A character string indicating whether "binary", "continuous" or "timetoevent"
#' data are used.
#' @param effsize A character string indicating underlying summary measure, it can be
#' "RR", "OR" for binary data type, "MD", "SMD" for continuous data type and "HR" for
#' timetoevent data type.
#' @param tau.sq An optional value for the between study variance tau^2
#' @param delta An optional value for the alternative effect sizes. If not specified, network
#' estimates are taken as alternative effects.
#' @return An object of class livenma
#' @return output A summary of the last pairwise and network meta-analysis results and
#' decisions made on the last step of the analysis
#' @return output2 A summary of the SUCRA values on the last step of the analysis
#' @return Prosp A matrix of all the results in the entire updating process
#' @return CompNTw Comparisons for which network estimates are available
#' @return D A data frame summarising initial data
#' @return frn id of first study for which results are displayed
#' @export

livenma<-function(data,level,type,effsize,tau.sq=NA,delta=NA){

#transform the data to usable format
  ...
  DataInput<-function(data,level,type,effsize){
    ...
    invisible(list(D=D, data=data, level=level, type=type, effsize=effsize))
  }

  d<-DataInput(data,level,type, effsize)

#perform pairwise and network meta-analysis
  ...
  metaanalysis<-function(D,tau.sq,type,level,effsize){
    ...
    CompNtw<-paste(PotComB[,2],rep("vs",length(PotComB)/2),PotComB[,1])
    CompNtwA<-paste(PotComB[,1],rep("vs",length(PotComB)/2),PotComB[,2])

    noc=factorial(metaNetw$n)/(factorial(2)*factorial(metaNetw$n-2)) #nu of possible compar
    noc==length(CompNtw)
    ...
    if (Tr>2){
      u1=u2=rep(0,Tr-1)
      u2[1]=Tr-1
      for (i in 2:(Tr-1)) {u2[i]=u2[i-1]+(Tr-i)}
      u1[1]=1
      for (i in 2:(Tr-1)) {u1[i]=u1[i-1]+(Tr-(i-1))}

      for (i in 1:(Tr-1)){
        Mntw[u1[i]:u2[i]]=(metaNetw$TE.random[i,][(i+1):Tr])
        seMntw[u1[i]:u2[i]]=(metaNetw$seTE.random[i,][(i+1):Tr])
      }
    }
    ...
    invisible(list(Tr=Tr,CompPairw=CompPairw,Mpairw=round(Mpairw,3), seMpairw=round(seMpairw,3),
               CompNtw=CompNtw,CompNtwA=CompNtwA,Mntw=round(Mntw,3), seMntw=round(seMntw,3)))
  }

  mafinal<-metaanalysis(D=d$D,tau.sq,level=d$level,type=d$type,effsize =d$effsize)
  if (is.na(delta)){
    delta=mafinal$Mntw
  }
  ma<-metaanalysis(D=d$D,tau.sq,level=d$level,type=d$type,effsize =d$effsize)

#prepare the output of meta-analyses for sequential analysis

  MaToInput<-function(ma,mafinal){

    CompPairw1=Mpairw1=seMpairw1=CompNetw1=Mnetw1=seMnetw1=rep(0,length(mafinal$CompNtw))
    ...
    input=data.frame(CompPairw1,Mpairw1,seMpairw1,CompNetw1,Mnetw1,seMnetw1,delta)
    colnames(input) <- c("Comparison Pairw","DirectTE","DirectSE","Comparison Netw","NetworkTE","NetworkSE","delta")
...
#perform sequential pairwise and network meta-analysis
  ...
  prospective<-function(Tr,input){
    ...
    ImaxPairw=ImaxNMA=abs((-qnorm(0.05/(2), 0, 1, lower.tail=TRUE, log.p=FALSE)
                   +qnorm(1-0.1, 0, 1, lower.tail=TRUE, log.p=FALSE))/(input$delta))

    #fraction of the maximum information accumulated 
    ...
    alpha <-
      function(method, t){
        ...
        E=qnorm(1-at/2, mean = 0, sd = 1, lower.tail = TRUE, log.p = FALSE)
    ...
    adjVarMntw=adjSE^2
    x<-mapply(rnorm,mean=input$NetworkTE[1:(Tr-1)],sd=adjVarMntw[1:(Tr-1)],n=draws) #modify for other ref
    ...
    #output results
    output=data.frame(input, Zpairw, Ipairw, Zntw, Intw, tallPairw, tallNMA,
                      AtPairw,EbPairw,AtNMA,EbNMA,LrciPairw,UrciPairw,LrciNMA,UrciNMA,FuPairw,FuNMA)

    colnames(output) <- c("ComparisonPairw","DirectTE","DirectSE","ComparisonNetw","NetworkTE","NetworkSE","delta",
                          "DirectZscore","DirectI","NetworkZscore","NetworkI","DirectT","NetworkT","DirectAlpha","DirectEfficacyB",
                          "NetworkAlpha","NetworkEfficacyB","DirectLowerRCI","DirectUpperRCI","NetworkLowerRCI","NetworkUpperRCI",
                          "DirectFutile","NetworkFutile")

    output2=data.frame(seq(1,Tr),c("Reference",as.character(output$ComparisonNetw[1:Tr-1])),SUCRAmr)
    colnames(output2) <- c("TreatID","Treatment(viaComparison)","SUCRA")
...
#summary of results
  ...
  Prosp=array(data=NA,dim=c(length(mafinal$CompNtw),23,((max(D$idyear)+1-frn+1))))

  for (i in frn:(max(D$idyear)+1)){
    ma<-metaanalysis(subset(D,idyear<i),tau.sq,type,level,effsize)
    invisible(ma)
    MaToInput1<-MaToInput(ma,mafinal)
    input<-MaToInput1$input
...
#output
  ...
  result=invisible(list(output=output, comparison=mafinal$CompNtwA, D=D, output2=output2, Prosp=Prosp, frn=frn))

  class(result) <- "livenma"
